(() => {
  ...
  const state = {
    roster: [], // [{id,name,payType,hourlyWage,monthlyBase,allowMaterial,allowForklift,allowLeader,allowDeputy}]
    byDate: {},
    undoStack: [],
  ...
  /**
   * ✅ 최종:
   * basePay(기본급) + timeExtraPay(연장/심야) + allowancePay(개인수당) = totalPay(총지급액)
   */
  function calcForPersonOnDate(person, rec) {
    ...
    // 연차/반차/결근은 기본급/수당 계산 0 (회사 규정 다르니 필요시 조정)
  ...
  function render() {
    ...
    for (const p of state.roster) {
      ...
      const meta = [
        ...
        `근무시간: ${calc.workMin}분 (${hhmm(calc.workMin)})`,
        ...
        `시간수당(연장/심야): OT1 ${money(calc.payOT1)} + OT2 ${money(calc.payOT2)} + 심야가산 ${money(calc.payNightExtra)} = ${money(calc.timeExtraPay)}원`,
        `개인수당: 자재 ${money(a.allowMaterial)} / 지게차 ${money(a.allowForklift)} / 조장 ${money(a.allowLeader)} / 반장 ${money(a.allowDeputy)} (합 ${money(calc.allowancePay)})`,
        `💰 총 지급액(기본+시간수당+개인수당): ${money(calc.totalPay)}원`,
      ].join("\n");
      ...
      el.querySelector(`#save-${CSS.escape(p.id)}`).addEventListener("click", () => {
        ...
        const am = el.querySelector(`#am-${CSS.escape(p.id)}`).value;
        const af = el.querySelector(`#af-${CSS.escape(p.id)}`).value;
        const al = el.querySelector(`#al-${CSS.escape(p.id)}`).value;
        const ad = el.querySelector(`#ad-${CSS.escape(p.id)}`).value;
        ...
        setPayField(p.id, "allowMaterial", num(am));
        setPayField(p.id, "allowForklift", num(af));
        setPayField(p.id, "allowLeader", num(al));
        setPayField(p.id, "allowDeputy", num(ad));
